...
def store_data(protocols):
    if protocols:
        ...
        for protocol in protocols:
            if isinstance(protocol['chains'], list):
                protocol['chains'] = ','.join(protocol['chains'])
            if isinstance(protocol.get('chainTvls'), dict):
                protocol['chainTvls'] = ','.join([f"{k}:{v}" for k, v in protocol['chainTvls'].items()])
