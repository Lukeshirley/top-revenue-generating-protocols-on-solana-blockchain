...
if 'result' in data:
    ...
    # Convert 'data' column to string directly
    df['total_revenue'] = df['data'].astype(str)

    # Group by executing_account and concatenate the total_revenue strings
    grouped_df = df.groupby('executing_account')['total_revenue'].apply(lambda x: ''.join(x)).reset_index()
